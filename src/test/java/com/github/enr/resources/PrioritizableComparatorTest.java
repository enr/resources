...
class PrioritizableComparatorTest {
  ...
  @ParameterizedTest
  @CsvSource({
      "10, 5, 5",   // o1.priority() > o2.priority() -> positive
  ...
  void compare_shouldReturnCorrectOrder(int priority1, int priority2, int expectedResult) {
    ...
    assertThat(result).as("compare(%d, %d)").isEqualTo(expectedResult);
  }

  @Test
  void compare_shouldReturnPositive_whenFirstHasHigherPriority() {
    ...
    assertThat(result).as("compare high priority vs low priority").isPositive();
  }

  @Test
  void compare_shouldReturnNegative_whenFirstHasLowerPriority() {
    Prioritizable lowPriority = createMockPrioritizable(1);
    Prioritizable highPriority = createMockPrioritizable(10);
    
    int result = comparator.compare(lowPriority, highPriority);
    assertThat(result).as("compare low priority vs high priority").isNegative();
  }
